...
Sheet is PUBLIC (Anyone with the link → Viewer).
Data is fetched as CSV — no API keys needed.
"""

from flask import Flask, Response, request as flask_request
import os, base64, pathlib, urllib.request, urllib.error, csv, io

app = Flask(__name__)

# ── Sheet config ──────────────────────────────────────────────────────────────
# Your Sheet ID (from the spreadsheet URL)
SHEET_ID = os.environ.get("SHEET_ID", "1vT0pZZbWovxsUYq9-rHlQgIN7kdG-bh2iXqlmb7AKBE")

# COLUMN INDICES — 0=A, 1=B, 2=C, 3=D ...
# Google Forms puts Timestamp in col A(0), then your questions follow.
# If your form asks: Q1=Name, Q2=Amount → NAME_COL=1, AMOUNT_COL=2
...
_b64_img   = base64.b64encode((_base/"atv.png").read_bytes()).decode()  if (_base/"atv.png").exists()  else ""
_b64_logo  = base64.b64encode((_base/"logo.png").read_bytes()).decode() if (_base/"logo.png").exists() else ""
...
def fetch_csv(sheet_id):
    """Download the sheet as CSV and return raw bytes."""
...
def parse_sponsors(raw_bytes, name_col, amount_col):
    """
    Parse CSV bytes into a sorted list of {name, amount} dicts.
    Returns (sponsors_list, debug_info_string).
    """
    ...
    header = rows[0]
    debug  = f"Header row ({len(header)} cols): {header}\n"
    debug += f"Total rows (incl header): {len(rows)}\n"
    debug += f"Using NAME_COL={name_col}, AMOUNT_COL={amount_col}\n"

    if len(rows) > 1:
        debug += f"First data row: {rows[1]}\n"

    out = []
    for i, row in enumerate(rows[1:], start=2):
        try:
            name       = str(row[name_col]).strip()
            amount_raw = (str(row[amount_col])
                          .replace(",", "")
                          .replace("₹", "")
                          .replace("Rs.", "")
                          .replace("Rs", "")
            ...
            debug += f"  Row {i} skipped ({e}): {row}\n"

    out.sort(key=lambda x: x["amount"], reverse=True)
    debug += f"Valid sponsors found: {len(out)}"
    return out, debug


def get_sponsors():
    try:
        raw      = fetch_csv(SHEET_ID)

        # Detect accidental HTML redirect (login page)
        if b"<!DOCTYPE" in raw[:300] or b"<html" in raw[:300]:
            print("ERROR: Got HTML instead of CSV. Sheet may not be public.")
            return DEMO, "Sheet returned HTML — make sure it is set to 'Anyone with link → Viewer'"

        sponsors, debug = parse_sponsors(raw, NAME_COL, AMOUNT_COL)
        print(debug)   # visible in Vercel function logs
        return sponsors if sponsors else DEMO, debug

    except urllib.error.HTTPError as e:
        msg = f"HTTP {e.code} fetching sheet"
        print(msg)
        return DEMO, msg
...
# ── /debug route — visit this URL to see exactly what the sheet returns ───────
@app.route("/debug")
def debug_route():
    lines = ["<pre style='font:14px monospace;padding:2rem;background:#111;color:#0f0'>"]
    lines.append(f"SHEET_ID   = {SHEET_ID}\n")
    lines.append(f"NAME_COL   = {NAME_COL}\n")
    lines.append(f"AMOUNT_COL = {AMOUNT_COL}\n\n")
    try:
        raw = fetch_csv(SHEET_ID)
        if b"<!DOCTYPE" in raw[:300] or b"<html" in raw[:300]:
            lines.append("❌ Google returned an HTML page (login redirect).\n")
            lines.append("Fix: Share → Anyone with the link → Viewer\n")
        else:
            sponsors, debug = parse_sponsors(raw, NAME_COL, AMOUNT_COL)
            lines.append(debug + "\n\n")
            lines.append("Parsed sponsors:\n")
            for s in sponsors:
...
def build_page(sponsors):
    atv_src  = f"data:image/png;base64,{_b64_img}"  if _b64_img  else ""
    logo_src = f"data:image/png;base64,{_b64_logo}" if _b64_logo else ""
...
.page{{position:relative;z-index:1;min-height:100vh;display:flex;flex-direction:column;align-items:center;padding:0 1rem 2rem}}
header{{text-align:center;padding:2.5rem 1rem 1rem;width:100%}}
.sovereign-logo{{width:clamp(80px,12vw,130px);height:auto;display:block;margin:0 auto .8rem;
...
@keyframes logoGlow{{
  ...
  to  {{filter:invert(1) brightness(2.0) drop-shadow(0 0 36px rgba(240,192,64,1.0));}}}}
.team-lbl{{font-family:'Bebas Neue',sans-serif;font-size:clamp(1rem,2.5vw,1.4rem);letter-spacing:.5em;color:var(--blue2);margin-bottom:.2rem}}
.title{{font-family:'Bebas Neue',sans-serif;font-size:clamp(2.4rem,8vw,5.5rem);letter-spacing:.06em;line-height:1;
...
.phoenix-badge{{display:inline-block;margin-top:.7rem;font-family:'Cinzel',serif;
...
.revival{{margin-top:2.5rem;text-align:center;font-family:'Cinzel',serif;
  font-size:clamp(.85rem,2.2vw,1.3rem);letter-spacing:.3em;text-transform:uppercase;
  background:linear-gradient(90deg,var(--orange),var(--gold),var(--orange));background-size:200% auto;
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;
...
footer{{margin-top:1.8rem;text-align:center;font-size:.7rem;color:var(--silver);opacity:.4;letter-spacing:.15em}}
@keyframes fadeUp{{from{{opacity:0;transform:translateY(24px)}}to{{opacity:1;transform:translateY(0)}}}}
@media(max-width:480px){{.stats{{gap:1rem}}.stat{{padding:.6rem 1rem}}.collage{{gap:.4rem .8rem;padding:1.2rem 1rem}}}}
</style>
</head>
<body>
<div class="bg">
  <img src="{atv_src}" alt="Team Sovereign Phoenix"/>
  <div class="bg-ov"></div>
</div>
<div class="page">
  <header>
